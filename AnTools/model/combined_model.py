...
class CombinedModelV2(nn.Module):
    def __init__(self, 
        ...
        self.up1 = NoPromptUpScaleModule(c_in=512, c_out=256)
        self.up2 = NoPromptUpScaleModule(c_in=256, c_out=128)
        self.up3 = NoPromptUpScaleModule(c_in=128, c_out=64)
        self.up4 = NoPromptUpScaleModule(c_in=64, c_out=32)
    ...
    def forward(self, query_batch, frame_batch):
        ...
        # (e.g., [B, 48, H/4, W/4], [B, 96, H/8, W/8], ...)
        frame_features = self.fastvit_backbone(frame_batch)
        frame_features = self.up1(frame_features[0], frame_features[1])
        frame_features = self.up2(frame_features, frame_features[2])
        frame_features = self.up3(frame_features, frame_features[3])
        frame_features = self.up4(frame_features, frame_features[4])
